...
export class AlbumComponent implements OnInit, OnDestroy {
    ...
    this.subscription = this.route.params.subscribe((params: any) => {
      ...
      this.getAlbum(albumId).subscribe(
        (album: AlbumInfo) => {
          ...
          this.numbers = Array(album.length)
            .fill(0)
            .map((x, i) => i + 1);
          this.album = of(album);
          this.mAlbum = album;
  ...
  nextPhoto() {
    if (this.mAlbum) {
      this.fullsScreenId = this.numbers[
        this.fullsScreenId % this.mAlbum.length
      ];
  ...
  previousPhoto() {
    if (this.mAlbum) {
      this.fullsScreenId = this.numbers[
        (((this.fullsScreenId - 2) % this.mAlbum.length) + this.mAlbum.length) %
          this.mAlbum.length
      ];
      const src = this.mAlbum.path + "/" + this.fullsScreenId + ".png";
